...
type TreePIRHintReq struct {
	RandSeed           PRGKey
	NumHintsMultiplier int
}

type TreePIRHintResp struct {
	NRows     int
	RowLen    int
	SetSize   int
	SetGenKey PRGKey
...
//unchanged 
func NewTreePIRHintReq(randSource *rand.Rand) *TreePIRHintReq {
	req := &TreePIRHintReq{
		RandSeed:           PRGKey{},
...
func (req *TreePIRHintReq) Process(db StaticDB) (HintResp, error) {
...
func (req *TreePIRHintReq) LocalityProcess(db StaticDB) (HintResp, error) {
...
func (resp *TreePIRHintResp) InitClient(source *rand.Rand) Client {
	c := treePIRClient{
		randSource: source,
		nRows:      resp.NRows,
		RowLen:     resp.RowLen,
...
//unchanged
func (resp *TreePIRHintResp) NumRows() int {
	return resp.NRows
}
...
func (c *treePIRClient) bernoulli(nHeads int, total int) bool {
	coin := c.randSource.Intn(total)
	return coin < nHeads
}
func (c *treePIRClient) sample(odd1 int, odd2 int, total int) int {
	coin := c.randSource.Intn(total)
	if coin < odd1 {
		return 1
...
func (c *treePIRClient) findIndex(i int) (setIdx int) {
...
type TreePIRQueryReq struct {
	PuncturedSet PuncturedSet
	ExtraElem    int
}

type TreePIRQueryResp struct {
	Answer    []byte
	ExtraElem Row
}

type treePIRQueryCtx struct {
	randCase int
	setIdx   int
	valPos int
}
...
func (c *treePIRClient) Query(i int) ([]QueryReq, ReconstructFunc) {
	if len(c.hints) < 1 {
		panic("No stored hints. Did you forget to call InitHint?")
	}
	var ctx treePIRQueryCtx
	...
	return []QueryReq{
			&TreePIRQueryReq{PuncturedSet: puncSetL, ExtraElem: extraL},
			&TreePIRQueryReq{PuncturedSet: puncSetR, ExtraElem: extraR},
		},
		func(resps []interface{}) (Row, error) {
			queryResps := make([]*TreePIRQueryResp, len(resps))
			var ok bool
			for i, r := range resps {
				if queryResps[i], ok = r.(*TreePIRQueryResp); !ok {
					return nil, fmt.Errorf("Invalid response type: %T, expected: *TreePIRQueryResp", r)
				}
			}
			//look at reconstruct?
...
//unchanged
func (c *treePIRClient) eval(setIdx int) PuncturableSet {
...
//unchanged 
func (c *treePIRClient) setGenForSet(setIdx int) *SetGenerator {
	if c.sets[setIdx].id < c.origSetGen.num {
		return &c.origSetGen
	} else {
		return &c.setGen
...
func (q *TreePIRQueryReq) Process(db StaticDB) (interface{}, error) {



	resp := TreePIRQueryResp{Answer: /*make(Row, db.RowLen)}*/make([]byte, (q.PuncturedSet.SetSize+1)*db.RowLen)}
...
func (c *treePIRClient) reconstruct(ctx treePIRQueryCtx, resp []*TreePIRQueryResp) (Row, error) {
	if len(resp) != 2 {
...
//
func (c *treePIRClient) NumCovered() int {
...
func (c *treePIRClient) randomMemberExcept(set PuncturableSet, idx int) int {
	for {
...
//not used
func (c *treePIRClient) StateSize() (bitsPerKey, fixedBytes int) {
